...
async function loadInventory() {
    try {
        ...
        if (response.ok) {
            ...
            console.error(`Failed to load inventory: ${response.status} ${response.statusText}`, errorText);
            inventoryItems = [];
        }
...
function createNFTElement(item) {
    ...
    if (item.prices) {
        if (item.prices.STARS > 0) {
            displayPrice = `${item.prices.STARS}`;
        } else if (item.prices.TON > 0) {
            displayPrice = `<img src="https://ton.org/download/ton_symbol.png" style="width: 14px; height: 14px; margin-right: 4px;" alt="TON">${item.prices.TON}`;
        } else if (item.prices.RUB > 0) {
            displayPrice = `<img src="https://i.postimg.cc/4yxzyjPG/IMG-1244.png" style="width: 14px; height: 14px; margin-right: 4px;" alt="RUB">${item.prices.RUB}`;
        }
    } else {
        // Для совместимости со старым форматом
        const starsPrice = Math.ceil(item.price * 100);
        displayPrice = `${starsPrice}`;
    }
    ...
    div.innerHTML = `
        ...
        <button class="buy-btn" onclick="event.stopPropagation(); console.log('Клик по кнопке купить, ID:', ${item.id}); buyItem(${item.id})" ${(item.stock === 0 || item.stock === undefined) ? 'disabled' : ''}>
            <img src="https://i.postimg.cc/R0w1D7hk/IMG-1243.png" alt="Star" class="buy-btn-icon">
...
function createActivityElement(item) {
    ...
    if (item.paymentMethod) {
        switch (item.paymentMethod) {
            case 'STARS':
                priceDisplay = `<img src="https://i.postimg.cc/3N3f5zhH/IMG-1243.png" style="width: 12px; height: 12px; margin-right: 4px;" alt="Stars">${item.convertedPrice || item.prices?.STARS || Math.ceil(item.price * 100)}`;
                break;
            case 'TON':
                priceDisplay = `<img src="https://ton.org/download/ton_symbol.png" style="width: 12px; height: 12px; margin-right: 4px;" alt="TON">${item.convertedPrice || item.prices?.TON || item.price}`;
                break;
            case 'YOOMONEY':
                priceDisplay = `<img src="https://i.postimg.com/4yxzyjPG/IMG-1244.png" style="width: 12px; height: 12px; margin-right: 4px;" alt="ЮMoney">${item.convertedPrice || item.prices?.RUB || Math.ceil(item.price * 300)}`;
                break;
            case 'RUB':
                priceDisplay = `<img src="https://i.postimg.cc/4yxzyjPG/IMG-1244.png" style="width: 12px; height: 12px; margin-right: 4px;" alt="RUB">${item.convertedPrice || item.prices?.RUB || Math.ceil(item.price * 300)}`;
                break;
            default:
                priceDisplay = `<img src="https://i.postimg.cc/3N3f5zhH/IMG-1243.png" style="width: 12px; height: 12px; margin-right: 4px;" alt="Stars">${item.convertedPrice || Math.ceil(item.price * 100)}`;
                break;
        }
    } else {
    ...
    // Правильная нумерация: считаем количество покупок этого товара до текущей покупки
    const currentItemIndex = activityItems.indexOf(item);
    const buyersForThisItem = activityItems.slice(currentItemIndex).filter(a => a.id === item.id).length;
    ...
    div.innerHTML = `
        ...
        <div class="activity-info">
            ...
            <div class="activity-id">#${item.buyerNumber || buyersForThisItem}</div>
        </div>
...
function openInventoryModal(item) {
    ...
    if (item.convertedPrice && !isNaN(item.convertedPrice) && item.convertedPrice > 0) {
        ...
        if (item.prices.STARS > 0) {
            ...
            priceDisplay = `<img src="https://i.postimg.cc/4yxzyjPG/IMG-1244.png" style="width: 14px; height: 14px; margin-right: 4px;" alt="RUB">${item.prices.RUB}`;
        }
    } else if (item.price && !isNaN(item.price) && item.price > 0) {
    ...
    modalId.textContent = `#${item.buyerNumber ? item.buyerNumber : (inventoryItems.filter(inv => inv.id === item.id && inv.userId <= item.userId).length)}`;
    ...
    loadInventory().then(() => {
        ...
        if (currentTransferItem.convertedPrice && !isNaN(currentTransferItem.convertedPrice) && currentTransferItem.convertedPrice > 0) {
            ...
            if (currentTransferItem.prices.STARS && currentTransferItem.prices.STARS > 0) {
                ...
                transferPriceDisplay = `<img src="https://i.postimg.cc/4yxzyjPG/IMG-1244.png" style="width: 14px; height: 14px; margin-right: 4px;" alt="RUB">${currentTransferItem.prices.RUB}`;
            }
...
function openPurchaseModal(item) {
    ...
    // Убираем отображение ID из модального окна покупки
    const modalIdElement = document.getElementById('modalId');
    if (modalIdElement) {
        modalIdElement.style.display = 'none';
    }
...
function openInviteModal() {
    if (window.Telegram && window.Telegram.WebApp) {
        ...
        if (user) {
            ...
            const referralLink = `${baseUrl}?start=${user.id}`;
            ...
            tg.showPopup({
                ...
                if (buttonId === 'share') {
                    const shareUrl = `https://t.me/share/url?url=${encodeURIComponent(referralLink)}&text=${encodeURIComponent('🎁 Присоединяйся к MetaGift! Покупай и дари уникальные подарки в Telegram!')}`;
...
function upgradeToNFT() {
    if (window.Telegram?.WebApp?.showPopup) {
        try {
            window.Telegram.WebApp.showPopup({
                title: 'Улучшение в NFT',
                message: 'Будет доступна в этом месяце.',
                buttons: [{ type: 'ok', text: 'Понятно' }]
            });
        } catch (error) {
            console.log('Будет доступна в этом месяце');
        }
    } else {
        alert('Будет доступно в этом месяце');
    }
}
